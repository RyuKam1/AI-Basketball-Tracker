...
class BasketballDataExtractor:
    ...
    def extract_pass_data(self, passes: List[int], frame_numbers: List[int]) -> None:
        """
        Extract pass detection data.
        
        Args:
            passes: List of detected passes (list of integers where -1=no pass, 1=Team1 pass, 2=Team2 pass)
            frame_numbers: List of frame numbers
        """
        for frame_idx, pass_type in enumerate(passes):
            if pass_type != -1:  # Only process actual passes (not -1)
                pass_entry = {
                    'pass_id': len(self.pass_data),
                    'frame': frame_idx,
                    'team': pass_type,  # 1 for Team 1, 2 for Team 2
                    'timestamp': frame_idx / 30.0,
                    'pass_type': f'Team {pass_type} Pass'
                }
                self.pass_data.append(pass_entry)
    
    def extract_interception_data(self, interceptions: List[int], frame_numbers: List[int]) -> None:
        """
        Extract interception detection data.
        
        Args:
            interceptions: List of detected interceptions (list of integers where -1=no interception, 1=Team1 interception, 2=Team2 interception)
            frame_numbers: List of frame numbers
        """
        for frame_idx, interception_type in enumerate(interceptions):
            if interception_type != -1:  # Only process actual interceptions (not -1)
                interception_entry = {
                    'interception_id': len(self.interception_data),
                    'frame': frame_idx,
                    'team': interception_type,  # 1 for Team 1, 2 for Team 2
                    'timestamp': frame_idx / 30.0,
                    'interception_type': f'Team {interception_type} Interception'
                }
    ...
    def extract_team_possession_data(self, ball_aquisition: List[int], frame_numbers: List[int]) -> None:
        """
        Extract team ball possession data.
        
        Args:
            ball_aquisition: List of ball possession data per frame (list of integers where each value is player_id or -1 for no possession)
            frame_numbers: List of frame numbers
        """
        for frame_idx, player_id in enumerate(ball_aquisition):
            frame_num = frame_numbers[frame_idx] if frame_idx < len(frame_numbers) else frame_idx
            
            possession_entry = {
                'frame': frame_num,
                'player_with_ball': player_id,  # -1 means no player has ball
                'has_possession': player_id != -1,
                'timestamp': frame_num / 30.0
            }
    ...
    def save_to_json(self, filename: str = None) -> str:
        ...
        data = {
            ...
            'player_data': convert_numpy_types(self.player_data),
            'ball_data': convert_numpy_types(self.ball_data),
            'pass_data': convert_numpy_types(self.pass_data),
            'interception_data': convert_numpy_types(self.interception_data),
            'speed_data': convert_numpy_types(self.speed_data),
            'team_possession_data': convert_numpy_types(self.team_possession_data)
        }
